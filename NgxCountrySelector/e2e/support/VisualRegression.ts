...
/*
 * Opens the demo-application, optionally opens the dropdown of the country-selector, and compares the
...
export async function expectDemoToLookLikeBaseline(page: Page, openDropdown: boolean, baselineName: string): Promise<void> {
    ...
    const component: Locator = page.locator('ngx-country-selector');
    try {
    ...
    let referenceImageArea: Box = await boundingBoxOf(component, 'the country-selector');
    if (openDropdown) {
        /*
