...
def update_readme_picture(tf:TFCPS_CodeUnitSpecific_NodeJS_Functions):
    ...
    readme_image_path = os.path.join(tf.get_repository_folder(), "Other", "Reference", "Technical", "Images", "ThemeSwitcher.png")
    
    # Find the first available baseline screenshot: try light first, then dark
    baseline_dir = os.path.join(tf.get_codeunit_folder(), "Other", "Resources", "VisualRegressionBaselines")
    
    matches = glob.glob(os.path.join(baseline_dir, "*", "*", "theme-switcher-light.png"))
    if matches:
        source_image_path = matches[0]
    else:
        matches = glob.glob(os.path.join(baseline_dir, "*", "*", "theme-switcher-dark.png"))
        if matches:
            source_image_path = matches[0]
        else:
            raise FileNotFoundError("No baseline screenshot found for theme-switcher")
    
    # Open the source image
    with Image.open(source_image_path) as img:
        # Crop to the component area - for theme-switcher, we take the center area
        # The viewport is 800x300, and the component is roughly in the center
        width, height = img.size
        
        # Calculate crop box: remove top and bottom margins to focus on the component
        # Based on the demo page layout, we crop from y=80 to y=220
        left = 0
        top = 80
        right = width
        bottom = 220
