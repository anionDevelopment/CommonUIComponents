"""Runs the visual-regression-tests of MatDarkmodeToggleButton (test/visual_regression) inside a container.
...
image (see ".ScriptCollection/OCIImages/ImageDefinition.csv"), so that the screenshots the tests take - and
therefore the baseline-images they are compared with - are reproducible independently of the operating-system of the
...
# Relative to the flutter-package ("mat_darkmode_toggle_button"). Only this folder is passed to "flutter test", so
# that the normal (non-visual) testcases are never re-run here.
...
def run_visual_regression_tests(tf: TFCPS_CodeUnitSpecific_Flutter_Functions, update_baselines: bool) -> None:
    ...
    # The ".dart_tool"-folder of the package is hidden behind an own volume, so that "flutter pub get" (which has to
    # run inside the container, because a ".dart_tool"-folder which was generated on the host contains
    # host-specific/absolute paths) neither uses nor overwrites the one of the host. The volume is reused by the
    # following runs, so only the first run has to download the packages again.
    ...
    if result[0] != 0:
        ...
        raise ValueError(f"The visual-regression-tests of MatDarkmodeToggleButton failed while {verb} them (docker exit-code {result[0]}).")
