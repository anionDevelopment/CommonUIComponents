...
def run_testcases():
    ...
    tf.run_testcases("mat_darkmode_toggle_button")
    # The visual-regression-tests are excluded from the run above (see "dart_test.yaml" of the flutter-package) and
    # are run here instead, inside a container, so that their baseline-comparison is reproducible independently of
    # the operating-system this script runs on. See "VisualRegressionContainer.py".
