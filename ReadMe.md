...
The package contains a small demo-application in `MatDarkmodeToggleButton/mat_darkmode_toggle_button/example`, which shows the widget. It is started with `task rd`.

Beside the unit-tests there are visual-regression-tests, which compare a screenshot of a demo-page with a baseline-image. They run inside a container so that the result does not depend on the operating-system of the developer. After an intended change of the appearance the baseline-images are regenerated with `task uvrb`; the two images of this readme are generated by the same run.
