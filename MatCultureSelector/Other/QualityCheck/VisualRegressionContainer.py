"""Runs the visual-regression-tests of MatCultureSelector (test/visual_regression) inside a container.
...
_package_name: str = "mat_culture_selector"

# Relative to the dart-package ("mat_culture_selector"). Only this folder is passed to "flutter test", so that the
...
def run_visual_regression_tests(tf: TFCPS_CodeUnitSpecific_Flutter_Functions, update_baselines: bool) -> None:
    ...
    if result[0] != 0:
        ...
        raise ValueError(f"The visual-regression-tests of MatCultureSelector failed while {verb} them (docker exit-code {result[0]}).")
